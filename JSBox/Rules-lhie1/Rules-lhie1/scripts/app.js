  ...
  $("bodyView").add({
    ...
    views: [{
      ...
      props: {
        ...
        data: ['Surge Pro', 'Surge 2', 'Quantumult'],
...
function makeConf(params) {
  ...
  try {
    ...
    let surgepro = outputFormat === 'Surge Pro'
    ...
    let proxies = flatServerData.map(i => {
      ...
      return i.proxyLink.replace('https://github.com/lhie1/Rules/raw/master/SSEncrypt.module')
    ...
    let promiseArray = [
      ...
      rulesReplacement ? getAutoRules(rulesReplacement, onPgs, '成功取回替换配置') : getAutoRules(isQuan || pu.direct : pu.apple, onPgs, '成功取回APPLE规则'), // 1
      ...
      !ads || rulesReplacement ? emptyPromise(onPgs) : getAutoRules(pu.urlrewrite, onPgs, '成功取回URL Rewrite'), // 6
    ...
    Promise.all(promiseArray).then(v => {
      ...
      let seperateLines = function (content, rules = false) {
        ...
        if (rules && !surgepro && promiseArray.length > 10) {
          ...
          for (let i = 10; i < promiseArray.length; i++) {
            ...
            addRules = addRules.concat(v[i].split(/[\r\n]/g).map(i => {
              console.log('i', i);
              if (/^(.+?),(.+?),(.+)$/.test(i)) {
                return `${RegExp.$1 + ',' + RegExp.$2},${policy},${RegExp.$3}${!',force-remote-dns' : ''}`
              }else if (/^(.+?),(.+?)(?=$|\/\/|\#)/.test(i)) {
                return `${RegExp.$1},${RegExp.$2},${policy}${!',force-remote-dns' : ''}`
              }
              return i
            }))
          }
